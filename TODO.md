...
- [x] Sidebar row clicks + double-click expand: whole PDF row is click/keyboard-interactive (kills the py-1 dead zone); double-clicking a folder or PDF toggles its subtree, chevron + kebab stop dblclick propagation so rapid taps don't fight the row's toggle.
- [x] Auto-growing textarea (`src/components/shared/AutoGrowTextarea.tsx`) replaces the single-line inputs in SelectionPopup and ChatThread follow-up. Enter submits, Shift+Enter newlines, content grows up to 240 px before scrolling inside itself. SelectionPopup grows a Type: Chat / Note pill group (Note mode hides Context, tints border/icon/primary-button amber, always-enabled Create so Enter can open a blank note).

### 9.C — Auto-titles, search, anchor overhaul (DONE)
- [x] LLM auto-titles chats via the cheapest text model per provider (Haiku 4.5 / GPT-5.4 nano / Gemini 2.5 Flash Lite), fired in parallel with the first response. Highlighted text is sent alongside the question so referential prompts like "what is this?" produce topical titles. New `repo.renameChat` + `chatStore.renameChat` + `Chat.titleGenerated` flag persist + gate to once-per-chat.
- [x] Cross-document MiniSearch over chat titles + concatenated message bodies and note titles + markdown bodies. Lazy index build (only when query is non-empty) to avoid streaming-time thrash. Right panel grows a search input that swaps the filter pills for ranked results; clicking opens the right doc + chat/note. `useAllIndexableChats` / `useAllIndexableNotes` liveQueries.
- [x] Notes always start as "Untitled note"; click-to-edit title in `NotePanel` header; body edits no longer auto-derive the title.
